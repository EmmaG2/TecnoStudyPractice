...
public class NumberSpiral {
  ...
  public static void solve() {
    ...
    pw.println(answer(x, y));
  }

  public static long answer(long x, long y) {
    long max = Math.max(x, y);
    if (max % 2 == 0) {
      if (y == 1) return max * max;
      if (x < max) return answer(max, max) - (max - x);
      if (x == max) return (max * max - (y - 1));
    } else {
      if (x == 1) return max * max;
      if (y < max) return answer(max, max) - (max - y);
